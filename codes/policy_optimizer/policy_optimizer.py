...
class PolicyOptimizer(ABC):
    ...
    def _init(self,
        ...
        self.multi_entropy_weight = multi_entropy_weight
    ...
    def _init_loss_with_entropy(self) -> None:
        ...
        with tf.name_scope("losses"):
            self.neglogp, entropy, self.neglogp_with_step, self.mask, multi_entropy = \
                self.policy.make_neglogp_and_entropy(self.sampled_batch_ph, self.entropy_gamma)

            # Entropy loss
            self.entropy_loss = -self.entropy_weight * tf.reduce_mean(entropy, name="entropy_loss")
            self.multi_entropy_loss = -self.multi_entropy_weight * tf.reduce_mean(multi_entropy, name="multi_entropy_loss")

            loss = self.entropy_loss + self.multi_entropy_loss
    ...
    def _setup_summary(self) -> None:
        ...
        with tf.name_scope("summary"):
            ...
            tf.summary.scalar("path_entropy_loss", self.multi_entropy_loss)
    ...
    def _setup_policy_optimizer(self,
                                ...
                                # Loss hyperparameters
                                entropy_weight: float = 0.005,
                                multi_entropy_weight: float = 0.005,
                                entropy_gamma: float = 1.0) -> None:
        """Setup of the policy optimizer.
        """
        self._init(sess, policy, debug, summary, optimizer, learning_rate,
                   entropy_weight, multi_entropy_weight, entropy_gamma)
