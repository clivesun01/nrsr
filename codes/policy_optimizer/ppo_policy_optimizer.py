...
class PPOPolicyOptimizer(PolicyOptimizer):
    ...
    def __init__(self,
        ...
        super()._setup_policy_optimizer(sess,
                                        policy,
                                        debug,
                                        summary,
                                        optimizer,
    ...
    def _set_loss(self):
        with tf.name_scope("losses"):
            ...
            clipped_ratio = tf.clip_by_value(ratio, 1. - self.ppo_clip_ratio, 1. + self.ppo_clip_ratio)
            ppo_loss = -tf.reduce_mean((r - self.baseline) * tf.minimum(ratio, clipped_ratio))

            # reward decay ppo loss
            r_decay = self.sampled_batch_ph.rewards_decay
            self.old_neglogp_step_ph = tf.placeholder(dtype=tf.float32, shape=(None, None), name="old_neglogp_step")
            ratio = tf.exp(self.old_neglogp_step_ph - self.neglogp_with_step)
            clipped_ratio = tf.clip_by_value(ratio, 1. - self.ppo_clip_ratio, 1. + self.ppo_clip_ratio)
            ppo_loss_rd = tf.reduce_sum((r_decay - self.baseline) * tf.minimum(ratio, clipped_ratio) * self.mask, axis=1)
            self.ppo_loss_rd = -tf.reduce_mean(ppo_loss_rd)

            # self-adaptive entropy adjust
            # entropy_alpha = 1.5 * logistic_function(ratio, 1)
            # self.loss *= entropy_alpha

            # Loss already is set to entropy loss
            self.loss += self.ppo_loss_rd

            # Define PPO diagnostics
            clipped = tf.logical_or(ratio < (1. - self.ppo_clip_ratio), ratio > 1. + self.ppo_clip_ratio)
    ...
    def _preppend_to_summary(self):
        with tf.name_scope("summary"):
            tf.summary.scalar("ppo_loss", self.ppo_loss_rd)
    ...
    def train_step(self, baseline, sampled_batch):
        ...
        old_neglogp_with_step = self.sess.run(self.neglogp_with_step, feed_dict=feed_dict)
        ...
        for ppo_iter in range(self.ppo_n_iters):
            ...
            for i, mb in enumerate(minibatches):
                sampled_batch_mb = Batch(
                    **{name: array[mb] for name, array in sampled_batch._asdict().items()}
                )
                mb_feed_dict = {
                    self.baseline: baseline,
                    self.batch_size: len(mb),
                    self.old_neglogp_ph: old_neglogp[mb],
                    self.old_neglogp_step_ph: old_neglogp_with_step[mb],
