...
class HierarchicalTask(Task):
    ...
    def get_next_obs(self, actions, obs, already_finished, var_mask=None):
        ...
        if var_mask is not None:
            l_var_mask = len(var_mask)
            add_var_mask = np.tile(var_mask, (prior.shape[0], 1))
            prior[:, :l_var_mask] += add_var_mask
