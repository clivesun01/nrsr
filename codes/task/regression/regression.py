...
class RegressionTask(HierarchicalTask):
    ...
    def __init__(self,
        ...
        if isinstance(dataset, dict):
            benchmark = BenchmarkDataset(**dataset, n_noise_var=n_redundancy)
            ...
            self.X_train_mask = benchmark.X_train_mask
            self.y_train_mask = benchmark.y_train_mask

            self.data_var_mask = benchmark.data_var_mask
            ...
            if function_set is not None and function_set != benchmark.function_set:
                print("WARNING: function_set provided when running benchmark problem. "
                      "The provided function_set will be ignored; "
                      "the benchmark function_set will be used instead.\n"
...
def make_regression_metric(name, y_train, *args):
    ...
    all_metrics = {
        ...
        "inv_nmse": (lambda y, y_hat: 1/(1 + args[0] * np.mean((y - y_hat) ** 2) / var_y), 1),

        # (Protected) inverse normalized root mean squared error
        # Range: [0, 1]
        # Value = 1/(1 + args[0]) when y_hat == mean(y)
        "inv_nrmse": (lambda y, y_hat: 1 / (1 + args[0] * np.sqrt(np.mean((y - y_hat) ** 2) / var_y)), 1),

        # Fraction of predicted points within p0*abs(y) + p1 band of the true value
        # Range: [0, 1]
        "fraction": (lambda y, y_hat: np.mean(abs(y - y_hat) < args[0]*abs(y) + args[1]), 2),
    ...
    assert len(args) == all_metrics[name][1], \
        "For {}, expected {} reward function parameters; received {}.".format(name,all_metrics[name][1], len(args))
