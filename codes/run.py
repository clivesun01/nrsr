...
from dso.core import MaskOptimizer
...
def train_mask(config):
    ...
    # Train the model
    model = MaskOptimizer(deepcopy(config))
    var_mask = model.run()

    print("== TRAINING MASK END ==============")

    return var_mask


def train_dso(config, var_mask=None):
    ...
    # Train the model
    model = DeepSymbolicOptimizer(deepcopy(config), var_mask)
    start = time.time()
    result = model.train()
...
def main(config_template, runs, n_cores_task, seed, benchmark, exp_name):
    ...
    if seed is not None:
        if config["experiment"]["seed"] is not None:
            messages.append(
                "INFO: Replacing config seed {} with command-line seed {}.".format(config["experiment"]["seed"], seed))
    ...
    if n_cores_task > 1:
        ...
        for i, config in enumerate(configs):
            result, summary_path = train_dso(config, var_mask)
            if not safe_update_summary(summary_path, result):
                print("Warning: Could not update summary stats at {}".format(summary_path))
            print("INFO: Completed run {} of {} in {:.0f} s".format(i + 1, runs, result["t"]))
