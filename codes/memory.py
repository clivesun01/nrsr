...
Batch = namedtuple(
    "Batch", ["actions", "obs", "priors", "lengths", "rewards", "rewards_decay", "on_policy"])
...
def get_samples(batch, key):
    ...
    batch = Batch(
        ...
        rewards_decay=batch.rewards_decay[key],
...
class ProgramQueueMixin():
    ...
    def _make_batch(self, samples):
        ...
        rewards_decay = np.array([s.rewards_decay for s in samples], dtype=np.float32)
