...
class DeepSymbolicOptimizer():
    ...
    def __init__(self, config=None, var_mask=None):
        ...
        self.config_redundancy = self.config["redundancy"]

        self.sess = None
        self.var_mask = var_mask

        # delete redundancy with the trained mask
        if self.var_mask is not None:
            self.redundancy_number = np.count_nonzero(self.var_mask == -np.inf)
            self.config_task['n_redundancy'] -= self.redundancy_number
            self.var_mask = self.var_mask[:-self.redundancy_number]

    def setup(self):
        # Clear the cache and reset to compute graph
        Program.clear_cache()
    ...
    def make_checkpoint(self):
        checkpoint = Checkpoint(self, **self.config_checkpoint)
        return checkpoint

    def make_policy_optimizer(self):
        policy_optimizer = make_policy_optimizer(self.sess, self.policy, **self.config_policy_optimizer)
...
class MaskOptimizer:
    ...
    def get_var_mask(self):
        from dso.l0_gating_module.data_set import RegressionDataset
        from dso.l0_gating_module.network import L0Network
        from dso.l0_gating_module.train_mask import Train

        dataset = RegressionDataset(self.data_var_mask)
        trainer = Train(dataset, L0Network)
        var_mask = trainer.run()
        var_mask = np.where(var_mask == 1, 0, -np.inf)
        print('======= The learned L0 Gates is : {} ======='.format(var_mask))
        return var_mask
    ...
    def run(self):
        ...
        # Genarate var mask
        if self.config_redundancy['train_redundancy_mask'] is True:
            self.data_var_mask = Program.task.data_var_mask  # alread has noise var
            var_mask = self.get_var_mask()
        else:
            var_mask = None
        return var_mask
