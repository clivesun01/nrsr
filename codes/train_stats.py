...
class StatsLogger():
    ...
    def __init__(self, sess, output_file, save_summary=False, save_all_iterations=False, hof=100,
        ...
        self.buffer_iter_entropy = BytesIO()  # Buffer for iteration entropy
    ...
    def setup_output_files(self):
        ...
        if self.output_file is not None:
            ...
            self.iteration_entropy_output_file = "{}_iteration_entropy.json".format(prefix)
            ...
            self.iteration_entropy_output_file = None
    ...
    def save_stats(self, r_full, l_full, actions_full, s_full, invalid_full, r,
                   ...
                   programs, positional_entropy, top_samples_per_batch, n_unique_programs, iteration_entropy):
        ...
        if self.save_positional_entropy:
            ...
            # np.save(self.iteration_entropy_output_file, np.array(iteration_entropy))
            with open(self.iteration_entropy_output_file, 'w') as f:
                json.dump(iteration_entropy, f)
    ...
    def save_results(self, pool, n_samples):
        ...
        result['n_unique_programs'] = self.n_unique_programs
        # all_programs = list(Program.cache.values())
        # str_programs = [p.str for p in all_programs]
        # len(set(str_programs))
    ...
    def flush_buffers(self):
        ...
        if self.save_positional_entropy:
            ...
            self.buffer_iter_entropy = self.flush_buffer(
                self.buffer_iter_entropy, self.iteration_entropy_output_file, byte_buffer=True)
