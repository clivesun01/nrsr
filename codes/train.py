...
class Trainer:
    ...
    def run_one_step(self, override=None):
        ...
        # that were generated during the attempt to get
        ...
        if self.logger.save_positional_entropy:
            ...
            self.iteration_entropy.append(float(np.mean(positional_entropy)))
        ...
        # r_var = np.var(r)
        # print('r_var: ', r_var)
        r_gamma_decay = r[:, np.newaxis]
        r_gamma_decay = r_gamma_decay * 1 ** np.arange(priors.shape[1])[::-1]
        ...
        if self.memory_queue is not None:
            self.memory_queue.push_batch(sampled_batch, programs)
        n_unique_programs = len(set(s_full))
        ...
        self.logger.save_stats(r_full, l_full, actions_full, s_full,
                               ...
                               n_unique_programs, self.iteration_entropy)
