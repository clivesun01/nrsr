...
class RNNPolicy(Policy):
    ...
    def __init__(self,
        ...
        print('The final action length is :', self.n_choices)
    ...
    def make_initializer(self, name):
        if name == "zeros":
    ...
    def make_cell(self, name, num_units, initializer):
        if name == 'lstm':
    ...
    def _setup_tf_model(
        ...
        with tf.name_scope("controller"):
            ...
            initializer = self.make_initializer(initializer)
            cell = tf.contrib.rnn.MultiRNNCell([self.make_cell(cell, n, initializer=initializer) for n in num_units])
            ...
            def loop_fn(time, cell_output, cell_state, loop_state):
                if cell_output is None:  # time == 0
                    ...
                    next_cell_state = cell.zero_state(batch_size=self.batch_size, dtype=tf.float32)
                    emit_output = None
                    ...
                    next_loop_state = (actions_ta, obs_ta, priors_ta, obs, prior, finished)
                else:
                    actions_ta, obs_ta, priors_ta, obs, prior, finished = loop_state

                    # apply bound to logits before applying prior, so that hard constraints are respected
                    ...
                    # Preventing training crashes
                    random_values = tf.random.uniform(shape=tf.shape(action), minval=0, maxval=self.n_choices, dtype=tf.int32)
                    action = tf.where(action >= self.n_choices, random_values, action)
                    ...
                    if self.var_mask is not None:
                        inp = [actions, obs, finished, self.var_mask]
                    else:
                    ...
                    finished = next_finished = tf.logical_or(next_finished, time >= max_length)
            ...
            memory_neglogp, _, _, _, _ = self.make_neglogp_and_entropy(self.memory_batch_ph, None)
    ...
    def make_neglogp_and_entropy(self, B, entropy_gamma):
        ...
        entropy_per_step = safe_cross_entropy(probs, logprobs, axis=2)  # -> (batch_size, max_length)
        entropy = tf.reduce_sum(entropy_per_step * entropy_gamma_decay_mask, axis=1)  # -> (batch_size, )

        # path entropy
        p_per_step = -safe_cross_entropy(actions_one_hot, probs, axis=2)  # Sum over action dim
        multi_p_per_step = tf.math.cumprod(p_per_step, axis=1)
        multi_logp_per_step = -tf.log(multi_p_per_step)
        multi_entropy = tf.reduce_sum(multi_p_per_step * multi_logp_per_step * mask, axis=1)  # -> (batch_size, )

        return neglogp, entropy, neglogp_per_step, mask, multi_entropy
    ...
    def apply_action_prob_lowerbound(self, logits):
        ...
        probs_bounded = (
                    (1 - self.action_prob_lowerbound) * probs + self.action_prob_lowerbound / float(self.n_choices))
