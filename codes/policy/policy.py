...
neglogp_with_step = tf.TensorArray
mask = tf.TensorArray
values = tf.TensorArray


def make_policy(sess, prior, state_manager, var_mask, policy_type, **config_policy):
...
class Policy(ABC):
    ...
    def make_neglogp_and_entropy(self, 
            B : Batch,
            entropy_gamma : float
            ) -> Tuple[neglogp, entropy, neglogp_with_step, mask]:
